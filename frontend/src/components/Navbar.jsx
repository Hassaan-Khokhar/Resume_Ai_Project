...
import { Home, Brain, Briefcase, LogOut, Sparkles, Search, X, User, MapPin } from 'lucide-react';
...
export default function Navbar() {
  ...
  const goToProfile = (id) => { setShowSearch(false); setQuery(''); navigate(`/profile/${id}`); };
  const goToJob = (id) => { setShowSearch(false); setQuery(''); navigate(`/jobs?highlight=${id}`); };
  const viewAll = () => { setShowSearch(false); navigate(`/search?q=${encodeURIComponent(query)}`); };
  ...
  return (
    <nav className="navbar">
      ...
      <div className="nav-user">
        ...
        <button className="btn-ghost" onClick={handleLogout}><LogOut size={16} /></button>
      </div>
    </nav>
  );
}
